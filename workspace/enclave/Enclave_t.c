...
static sgx_status_t SGX_CDECL sgx_process_lidar(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_process_lidar_t));
	//
	...
	if (_tmp_retptr != NULL && _len_retptr != 0) {
		if ( _len_retptr % sizeof(*_tmp_retptr) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_retptr = (float*)malloc(_len_retptr)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
...
static sgx_status_t SGX_CDECL sgx_say_something(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_lidar = ms->ms_lidar;
	size_t _tmp_points_num = ms->ms_points_num;
	size_t _len_lidar = _tmp_points_num * sizeof(uint8_t);
	uint8_t* _in_lidar = NULL;
	uint8_t* _tmp_hash = ms->ms_hash;
	size_t _len_hash = 64 * sizeof(uint8_t);
	uint8_t* _in_hash = NULL;

	if (sizeof(*_tmp_lidar) != 0 &&
		(size_t)_tmp_points_num > (SIZE_MAX / sizeof(*_tmp_lidar))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}
	...
	if (_tmp_lidar != NULL && _len_lidar != 0) {
		...
		_in_lidar = (uint8_t*)malloc(_len_lidar);
		if (_in_lidar == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	ms->ms_retval = say_something((const uint8_t*)_in_lidar, _tmp_points_num, _in_hash);
	if (_in_hash) {
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_process_lidar, 0, 0},
